...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('../raheem/cm');
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('../raheem/cm');
...
async function startRaven() {
  ...
  const client = ravenConnect({
    ...
    browser: ["RAHEEM-CMD", "Safari", "5.1.7"],
    auth: state,
  ...
  if (autobio === 'TRUE') {
    setInterval(() => {
      ...
      client.updateProfileStatus(
        `📅 𝙳𝙰𝚃𝙴/𝚃𝙸𝙼𝙴 ⌚️  ${date.toLocaleString('en-US', { timeZone: 'Africa/Dar es Salaam ' })}  ⏰️ 𝙳𝙰𝚈 ⏰️  ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Dar es Salaam '})}. RAHEEM 𝙸𝚂 𝙲𝚄𝚁𝚁𝙴𝙽𝚃𝙻𝚈 𝙰𝙲𝚃𝙸𝚅𝙴 𝙰𝙽𝙳 𝚁𝚄𝙽𝙽𝙸𝙽𝙶⚡.`
      );
    }, 10 * 1000);
  }
 ...
 client.ev.on('call', async (callData) => {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(color("Congrats, RAHEEM-CMD has successfully connected to this server", "green"));
      console.log(color("Follow me on Instagram as Nic.k_hunter", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      const Texxt = `✅ 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱  ╍>〚𝗥𝗔HEEM-CM〛\n`+`👥 𝗠𝗼𝗱𝗲  ╍>〚${mode}〛\n`+`👤 𝗣𝗿𝗲𝗳𝗶𝘅  ╍>〚 ${prefix} 〛`
